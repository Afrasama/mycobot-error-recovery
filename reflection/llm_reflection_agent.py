...
class LLMReflectionAgent:
    def __init__(
        ...
        self.endpoint = endpoint or os.getenv("LLM_AGENT_ENDPOINT") or self._default_endpoint()
    ...
    def _query_openai(self, scene_info, policy, rgb, history) -> LLMDecision:
        ...
        return self._parse_decision(data["choices"][0]["message"]["content"], mode="openai")

    def _query_ollama(self, scene_info, policy, rgb, history) -> LLMDecision:
        prompt = self._build_prompt(scene_info, policy, history)
        user_message = {"role": "user", "content": prompt}
        image_b64 = self._rgb_to_base64_png(rgb) if self.use_vision else None
        if image_b64:
        ...
        data = json.loads(response_text)
        return self._parse_decision(data["message"]["content"], mode="ollama")
    ...
    def _post_json(self, endpoint, payload, headers, label: str) -> str:
    ...
    def _system_prompt(self) -> str:
        return (
            ...
            "You may change x_offset, y_offset, grasp_height, approach_height, "
            "lift_height, and release_delay. "
            "Use small, targeted deltas, not absolute values."
        )
    ...
    def _build_prompt(self, scene_info, policy, history) -> str:
        ...
        return (
            ...
            "Rules:\n"
            "- Return JSON only.\n"
            "- All update values are deltas, not absolute values.\n"
            "- If a field should not change, omit it from updates.\n"
            "- Prefer the smallest useful correction.\n"
            "- Set terminate=true only if retrying is unlikely to help.\n"
            "- Use scene_info as the primary signal; use the image only if provided.\n\n"
    ...
    @staticmethod
    def _rgb_to_base64_png(rgb: Optional[np.ndarray]) -> Optional[str]:
        if rgb is None:
            return None
        try:
    ...
    def _rgb_to_openai_image_url(self, rgb: Optional[np.ndarray]) -> Optional[str]:
        image_b64 = self._rgb_to_base64_png(rgb)
