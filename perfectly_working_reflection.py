...
p.changeDynamics(cube, -1, lateralFriction=1.5)
...
p.resetDebugVisualizerCamera(
    ...
    cameraYaw=45,
    cameraPitch=-35,
    cameraTargetPosition=[0.30, 0.10, 0.08]
)
...
while p.isConnected():
    ...
    if state == "plan":
        ...
        initial_cube_pos = cube_pos.copy()
        perceived_cube_pos = cube_pos.copy()

        # ---- INJECT ERROR ONLY ON FIRST ATTEMPT ----
        if inject_failure:
    ...
    # -------- LIFT --------
        ...
        lift_target = initial_cube_pos.copy()
    ...
    # -------- OBSERVE --------
        ...
        if distance_to_goal < 0.05 and speed < 0.08:
            stable_counter += 1
        else:
            stable_counter = 0
    ...
    # -------- ANALYZE --------
        ...
        print("Analyzing failure... correcting perception")

        # disable failure after first analyze
